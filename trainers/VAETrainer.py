...
class VAETrainer:
    ...
    def train(cls, NO_EPOCHS):
        ...
        vae = VAE().to(device)
        if os.path.exists(cls.current_directory_path("vae_model.bin")):
            vae.load_state_dict(
                torch.load(
                    cls.current_directory_path("vae_model.bin"), map_location=device
                )
            )
        else:
            ...
            for epoch in range(NO_EPOCHS):
                for data_index in np.random.permutation(range(132)):
                    ...
                    for image_index in np.random.permutation(range(no_images)):
                        optim.zero_grad()
                        x = cls.hwc_to_chw(x_all[image_index]).to(device)
                        (x_hat, z_mean, z_logvar) = vae.forward(x, True)
                        l2_loss = cls.l2_norm_loss_fn(x, x_hat)
                        l2_loss_accum += l2_loss.item()
                        kl_loss = cls.kl_loss_fn(z_mean, z_logvar)
                    ...
                    torch.save(
                        vae.state_dict(), cls.current_directory_path("vae_model.bin")
                    )

        saved_file = np.load("data/data133.npz")
        x_all = torch.from_numpy(saved_file["x"])
        x = cls.hwc_to_chw(x_all[100]).to(device)
        x_hat, z_mean, z_logvar = vae.forward(x, False)
        original = (cls.chw_to_hwc(x) * 255).to(torch.uint8).cpu().numpy()
        reconstruction = (cls.chw_to_hwc(x_hat) * 255).to(torch.uint8).cpu().numpy()
